...
3. Запустите Chaos Link ярлыком, затем скопируйте из консоли сайт, код комнаты и **пароль друзей**. Обычный запуск не требует запроса UAC.
...
Стандартный журнал мастера создаётся в `%TEMP%` с именем `Setup Log*.txt`, журнал первоначальной настройки находится в `%LOCALAPPDATA%\ChaosLink\runtime\install-bootstrap.log`, а полный журнал запуска — в `runtime\startup.log`. Если программа сообщает об ошибке, приложите эти файлы вместе с соответствующими `*.err.log` к обращению в поддержку.
