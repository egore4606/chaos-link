...
- установщик собран стандартным Inno Setup, показывает каталог назначения и все выполняемые действия;
- установочный EXE уже содержит .NET, AutoHotkey и `cloudflared`, поэтому ничего не скачивает на компьютере пользователя;
- версии сторонних файлов зафиксированы, а их SHA-256 проверяется до упаковки;
- раннее логирование Inno Setup включено до копирования первого файла;
...
3. Подписывать одним издателем `ChaosLink-Setup.exe` (включая встроенный деинсталлятор) с SHA-256 и доверенной отметкой времени.
...
Сборка вызывает `scripts/sign-release.ps1`, проверяет подпись через SignTool и только затем создаёт `SHA256SUMS.txt`. Закрытый ключ и пароль сертификата не сохраняются в репозитории.
